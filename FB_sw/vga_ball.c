 ...
 */
 ...
 static long vga_ball_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
 {
     ...
     case VGA_BALL_FLAP:
         // 当收到flap命令时，减小y坐标（使球向上移动）
         dev.ball.y = dev.ball.y > 50 ? dev.ball.y - 30 : 20; // 防止飞出屏幕顶部
         // 写入新位置
         write_ball_position(&dev.ball);
         break;
 
     case VGA_BALL_UPDATE_GAME:
         // 更新游戏状态（例如球体自然下落）
         dev.ball.y += 5; // 简单的重力效果
         if (dev.ball.y > 480 - dev.ball.radius) // 防止飞出屏幕底部
             dev.ball.y = 480 - dev.ball.radius;
         write_ball_position(&dev.ball);
         break;
 
     case VGA_BALL_RESET_GAME:
         // 重置游戏
         dev.ball.x = 320;
         dev.ball.y = 240;
         write_ball_position(&dev.ball);
